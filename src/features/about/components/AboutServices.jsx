...
export const AboutServices = () => {
  ...
  return (
    <section
      ref={sectionRef}
      className="about-services bg-[#000000] text-white py-24 sm:py-36 border-b border-white/10 font-sans overflow-hidden relative"
    >
      {/* Background grid lines */}
      <div className="absolute inset-0 grid grid-cols-6 md:grid-cols-12 pointer-events-none z-0">
        {Array.from({ length: 12 }).map((_, idx) => (
          <div key={idx} className="border-r border-white/[0.03] h-full" />
        ))}
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-12 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start mb-16 sm:mb-24">
          ...
          <div className="svc-header-item md:col-span-7">
            <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-normal tracking-tight text-white leading-[1.18]">
              Creative Direction, Lasting Value
            </h2>
          </div>
        </div>

        {/* Services Grid — 2 cols desktop, 1 col mobile */}
        <div className="grid grid-cols-1 sm:grid-cols-2 border-t border-l border-white/10 bg-[#0a0a0a]">
          {services.map((svc) => (
            <a
              key={svc.id}
              href={svc.href}
              className="svc-card group relative flex flex-col justify-between p-8 sm:p-12 border-r border-b border-white/10 min-h-[260px] bg-[#000000] transition-all duration-500 hover:bg-white/[0.04] cursor-pointer"
            >
              <CornerBrackets size="w-3.5 h-3.5" color="border-white/30" />

              {/* Title + Arrow */}
              <div className="flex items-start justify-between gap-4">
                <p className="text-xl sm:text-2xl font-normal text-white group-hover:text-bordeaux transition-colors duration-300">
                  {svc.title}
                </p>
                <div className="shrink-0 w-10 h-10 border border-white/20 flex items-center justify-center group-hover:border-bordeaux group-hover:bg-bordeaux/10 transition-all duration-300 relative">
                  <ArrowUpRight className="w-5 h-5 -rotate-0 group-hover:-rotate-12 group-hover:text-bordeaux transition-all duration-300" />
                  <CornerBrackets size="w-2 h-2" color="border-white/40" />
                </div>
              </div>

              {/* Description */}
              <p className="mt-8 text-gray-400 font-light leading-relaxed text-base sm:text-lg group-hover:text-gray-200 transition-colors duration-300">
                {svc.description}
              </p>
            </a>
          ))}
